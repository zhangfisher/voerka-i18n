...
<div>
  <input type="text" placeholder="输入待办事项" on:keyup={e => e.key === 'Enter' && addTodo(e)}>
...
<ul class="todo-list">
  {#each $todos as todo (todo.id)}
    <li class="todo-item">
      ...
      <button on:click={() => addTodo()}>增加</button>
      <button on:click={() => updateTodo({ ...todo, done: !todo.done })}>{todo.done ? '已完成' : '未完成'}</button>
      <button on:click={() => updateTodo({ ...todo, text: '清空' })}>清空</button>
      <button on:click={() => updateTodo({ ...todo, text: '默认文本' })}>默认</button>
      <button on:click={() => updateTodo({ ...todo, text: '自定义文本' })}>自定义</button>
    </li>
  {/each}
</ul>
