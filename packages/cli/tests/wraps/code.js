...
async function translateMessages(messages={},from="zh",to="en",options={}){
    ...
    try{
        ...
        console.error(e)
        throw new Error('调用翻译API时出错:{}',e.message)
    }
...
async function translateMessageFile(file,langSettings,options={}){    
    let context = this
    logsets.log("正在翻译文件:{}",path.basename(file))
    ...
    for(let lng  of langSettings.languages){
        ...
        try{
            tasks.add("翻译 {} -> {}",[defaultLanguage,lng.name])
...
async function translate(srcFolder,opts={}){
    ...
    if(!provider && !(appkey && appid) ) throw new Error("需要指定翻译脚本或者appkey和appid")
...
program
    ...
    .description('调用在线翻译服务商的API翻译译指定项目的语言包,使用百度云翻译服务')
    .option('--no-backup', '备份原始文件')
    .option('--mode', '翻译模式，取值auto=翻译未翻译的,full=全部翻译', 'auto')
    .option('-p, --provider <value>', '在线翻译服务提供者名称或翻译脚本文件', 'baidu')
    .option('-m, --max-package-size <value>', '将多个文本合并提交的最大包字节数', 200)
    .option('--appid [id]', 'API IDD')
    .option('--appkey [key]', 'API密钥')
    .option('-q, --qps <value>', '翻译速度限制,每秒可调用的API次数', 1)  
