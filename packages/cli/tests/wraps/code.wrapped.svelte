...
<ul class="todo-list">
  {#each $todos as todo (todo.id)}
    <li class="todo-item">
      ...
      <button on:click={() => updateTodo({ ...todo, text: '清空' })}>{t('清空')}</button>
      <button on:click={() => updateTodo({ ...todo, text: '默认文本' })}>{t('默认')}</button>
      <button on:click={() => updateTodo({ ...todo, text: '自定义文本' })}>{t('自定义')}</button>
    </li>
  {/each}
</ul>
